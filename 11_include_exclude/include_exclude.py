...
if user is not None:
    ...
    ## include enables to have only have those field which you want to see in response
    #user_json = user.model_dump_json(include={"username", "email", "website", "first_name", "last_name", "bio", "follower_count"}, indent=2)    



    ### ABOUT exclude ####

    ## exclude enable to remove/omit the class fields/attributes which u don't want to see in the final JSON response/output.
    
    ## BITTER TRUTH: You cannot exclude fields using alias name directly. You have to use class field/attribute names while including or excluding fields.

    ## BEST WAY: It is always best to use class field/attribute names while including or excluding fields
    user_json = user.model_dump_json(exclude={"uid", "password", "verified_at", "is_active"}, indent=2)
...
if post is not None:
    ...
    post_json = post.model_dump_json(
        exclude={                                                       
            "author": {"uid", "password", "is_active", "verified_at", "is_influencer"},     
            "is_published": True,
            "created_at": True,
            "comments": {   
                "__all__": {"author_email", "likes"} 
            },
        },
        indent=2
    )
